...
5. 物理目录统一为 `skills/<category-path>/<skill-name>/`；skill 名称不变。外部安装入口以 junction（Windows）或 symlink 指向安装克隆 `~/.lfenskills` 中的该路径；目录移动或改名后，生成器会提醒并运行 `python scripts/check_install_links.py` 核对各入口。
