...
讨论并确认某个 Agent Skill 出了问题后，按固定 JSON 分开记录可核对观察与推理原因。落盘默认写到 `D:/skill_problem/<skill-name>/`，目录不存在则创建。创建实际文档前必须向人征求是否落盘和文件名。

**适用**：人已确认 skill 规范缺失、模型未执行规范、门禁只申报不走、或复查用派生物自验，并要求留档的时候。
