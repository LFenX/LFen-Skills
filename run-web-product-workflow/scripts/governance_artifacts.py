...
def validate_embedded_manifest(skill_root: Path | None = None) -> list[str]:
    ...
    required_runtime = {
        ...
        path.relative_to(root).as_posix()
        for path in (root / "assets" / "project-templates").glob("*.md")
    }
    listed: set[str] = set()
...
def build_audit_console(project_root: Path, *, project_id: str) -> list[Path]:
    """Write the human-readable audit console as a fourth DerivedView family.

    The DerivedView schema constrains neither content type nor view_kind, so an HTML
    page needs no schema change and still gets the same envelope, source list and
    snapshot digest as the markdown reviews. It is regenerable and, per C10 8.0, must
    never be the only source of a tracking fact.
    """
    ...
    sources = [state_path]
    graph = {node["task_id"]: node for node in state.get("task_graph", [])}
    status_by_id = {item["task_id"]: item for item in state.get("source_tasks", [])}

    tasks: list[dict[str, Any]] = []
    tasks_root = root / "tasks"
    for task_id, node in sorted(graph.items(), key=lambda kv: kv[1]["ordinal"]):
        ...
        entry: dict[str, Any] = {
            ...
            "status": status_by_id.get(task_id, {}).get("status", "Active"),
            "objective": "",
            "risk_level": "",
            "delivery_scenario": "",
            "development_types": [],
        ...
        if minimal_path.is_file():
            record = read_json(minimal_path)
            sources.append(minimal_path)
            profile = record.get("task_contract", {}).get("task_profile", {})
            ...
            entry.update(
                ...
                events=[
                    {"event_type": e.get("event_type", ""), "summary": e.get("summary", ""), "timestamp": e.get("timestamp", "")}
                    for e in record.get("run_ledger", [])
                ],
            )
            outcome = record.get("task_outcome") or {}
            entry["established_facts"] = outcome.get("established_facts", [])
            entry["actual_changes"] = outcome.get("actual_changes", [])
        elif before_path.is_file():
            before = read_json(before_path)
            sources.append(before_path)
            profile = before.get("task_profile", {})
            entry.update(
                carrier="完整载体",
            ...
            if run_path.is_file():
                sources.append(run_path)
                entry["events"] = [
                    {"event_type": e.get("event_type", ""), "summary": e.get("summary", ""), "timestamp": e.get("timestamp", "")}
                    for e in read_jsonl(run_path)
                ]
            after_path = task_dir / "after.json"
    ...
    manifest = read_json(manifest_path)
    mismatched = sorted(validate_embedded_manifest())
    ...
    payload = {
        ...
        "derivation_audit": audit_console.derivation_audit(state),
        "manifest": {"total": len(manifest.get("files", [])), "mismatched": mismatched},
    }
    content_path = root / "generated" / "reviews" / "audit-console.html"
    return _write_derived_view(
        root=root,
        content_path=content_path,
        content=audit_console.render(payload),
        project_id=project_id,
        view_id=f"DV-{project_id}-AUDIT-CONSOLE",
        view_kind="governance-audit-console",
