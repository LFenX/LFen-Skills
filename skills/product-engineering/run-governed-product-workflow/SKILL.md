...
2. 只向人询问会改变目标、范围、验收、风险或授权的信息；判据是猜错的后果，不是能否自行查出。无论问还是不问，都必须先按「需求澄清」明确告知用户本次是否跳过提问，不存在沉默跳过。
...
**载体层——只读一张命令卡。** 按下方路由表进入对应的 `commands/*.md`，不要顺带读其他卡。唯一例外是 [commands/clarify.md](commands/clarify.md)：任何路径在建立任务前都必须读它，它不是只有命令词 `clarify` 才进入的卡。
...
建立任务前无条件执行，与用户是否打命令词无关。

**先勘察。** 按初步理解定位相关代码，得出相关位置、现有实现、既有约定和分歧点。没有勘察结果不得声称需求清楚。

**再判定。** 命中任一即必须提问，与能否自行查出无关：目标用户/问题/期望行为/完成判定无法唯一表达；涉及安全、隐私、数据、不可逆、生产影响或 Authority 的未知；勘察发现多条都合理且选错需要重做的路径；会改变外部可见行为而期望行为未唯一确定；涉及授权、例外或风险接受。
...
用用户提问所用的语言表述，实际说出的那句原文写入 `clarification.notice`。

**归档。** 用户原话进 `request_snapshot`，澄清过程进 `clarification`：每轮的问题、它会改变的项、推荐默认、用户回答原话、回答状态。两者同等重要，都是验收基准。二者缺失、或 `clarification.state` 不是 `Settled`、或还有回答处于 `Ambiguous`/`Deferred` 时，`resolve_tailoring` 从 S2 起返回 blocker，任务无法进入二次确认与计划。
...
二次确认是执行停点，不是排版：澄清 `Settled` 后固定展示要解决的问题、期望结果、本次会做、本次不做、如何验收、主要风险或未知、采用的默认项，并以 Ask：Proceed / Revise / Stop 结束，等到答复才继续。
