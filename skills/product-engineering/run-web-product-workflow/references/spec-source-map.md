...
- 唯一编辑事实源：`LFen-Skills/skills/product-engineering/run-web-product-workflow`；规范正文、规范映射、Schema、评测集和 Skill 实现都只能在该处修订。本 Skill 自带完整规范，不镜像也不依赖任何外部规范仓库。
...
发布信任根是 `LFen-Skills/skills/product-engineering/run-web-product-workflow` 所在 Git 提交；维护者发布时建议使用 annotated tag，例如 `rwpw-v6.3.0-candidate`。消费方校验当前发布快照时运行：
