...
def scan_integrity() -> dict[str, Any]:
    """Check the Skill's protected assets, or say plainly that they are out of reach.

    This is the one page that audits the Skill rather than the project, so a console
    deployed into a project can only answer it while the Skill it came from is
    reachable. An unreachable Skill must not read as "0 assets, nothing wrong" -- a
    zero here means the check did not run, which is a different fact from a clean
    check and has to be shown as one.
    """

    manifest_path = embedded_manifest_path()
    if not manifest_path.is_file():
        return {
            "reachable": False, "total": 0, "errors": [], "roles": {},
            "source_of_truth": "", "manifest_path": str(manifest_path),
        }
    try:
        manifest = read_json(manifest_path)
    except (OSError, json.JSONDecodeError, GovernanceError) as exc:
        return {
            "reachable": True, "total": 0, "errors": [str(exc)], "roles": {},
            "source_of_truth": "", "manifest_path": str(manifest_path),
        }
    return {
        "reachable": True,
